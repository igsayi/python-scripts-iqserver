...
for i in range(len(df_combine)):
    if df_combine.loc[i, "_merge"] == "left_only" and df_combine.loc[i, "DaysToFix"] == -1:  # type: ignore  # This set of violations are closed today
        ...
        # print(i)
    elif df_combine.loc[i, "_merge"] == "right_only":  # type: ignore  # This set are newly opened today
        ...
        openDate = datetime.strptime(df_combine.loc[i, "openTime_y"][:10], "%Y-%m-%d")  # type: ignore
        df_combine.loc[i, "openTime_x"] = openDate
...
df_combine.drop(
    ["threatLevel_y", "policyName_y", "name_y", "OrgName_y", "appExposure_y", "displayName_y", "openTime_y", "CVE_y"], axis=1, inplace=True
)
df_combine.to_csv(
    f"output/iq_master-file-{fileDate}.csv",
    header=[
        "threatLevel",
        "policyName",
        "name",
