...
for app in apps:
    ...
    #print("AppName: "+ str(app["applicationTags"]))
    #print("AppCategory: " + appCatList.get(app["id"]))
    if appCatList.get(app["id"]) == "Existing":
        newTag = {}
        for appTag in app["applicationTags"]:
            if appTag["tagId"] == "f36242c553334ae6a4ac34ef952d33f9":   # New
                newTag = appTag
        if len(newTag) > 0:
            app["applicationTags"].remove(newTag)
            app["applicationTags"].append({"tagId": "e8e069027fdd46faa1791f794e3eced4"})    # Existing

        #print("index: " + app["applicationTags"].index({"tagId": "f36242c553334ae6a4ac34ef952d33f9"}))    # New

    elif appCatList.get(app["id"]) == "New":
        app["applicationTags"].append({"tagId": "f36242c553334ae6a4ac34ef952d33f9"})        # New
    else:
